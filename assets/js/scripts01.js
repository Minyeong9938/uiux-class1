...
// let 과 const 차이점
// let은 변수를 한번 선언 후 값을 변경할 수 있음.
// const는 변수를 선언 후 값을 변경할 수 없음. 상수를 선언할 때 사용함.
...
// 첫번째: 호이스팅
// 자바스크립트는 실행이 되기 전 선언한 변수를 미리 찾아서 보관해둔다. 변수만 보관하고 값은 보관하지 않음.
// 이것을 호이스팅이라고 한다.
...
// let a = 2;
// console.log(a);
// 에러가 발생함. 이미 선언을 했다고 얘기해줌


// 세번째: 지역변수와 전역변수

// 전역변수: 블럭 밖에서 선언된 변수. 어디서든 쓰일 수 있음
// 지역변수: 블럭 안에서 즉 특정 영역에서만 선언된 변수. 헤당 영역 내에서만 쓰일 수 있음
...
// var는 지역변수를 전역변수로 끌어올려준다.(for문, if문 등) 함수내에서 선언한 변수만 제외함.


// ============================================

// 변수 이름 만들 때
// 1. 숫자로 시작하면 안됨. $, _, 영문자로 시작
// 2. 대부분 변수는 소문자로 시작함.
...
let a = 1; //숫자. 정수와 실수 모두 가능
let b = "5"; //문지열. string. 따옴표로 감싸져 있음
// console.log(a);
// console.log(b);
// console.log(a + b);


// 불리언(boolean)타입. 참 또는 거짓. 논리연산에 사용되는 데이터 타입.
// true 또는 false로 사용
...
// % 나머지 연산자. 10 % 3 = 1 은 10을 3으로 나눈 나머지 1을 반환함
...
// num = num == 2;
// == 비교 연산자. 같은 값인지 비교를 하는 연산자. 1==2 false를 반환함.
// num = num === 2;
// === 비교 연산자. 값과 타입을 모두 비교함.

// == 과 ===의 차이점
// 1 == "1" true. 값만 비교함.
// 1 === "1" false. 값과 타입을 모두 비교함.
...
// && and 연산자. 두 값이 모두 true일 때만 true를 반환함.
// num = num > 2 && num < 10;
...
// num = !num;
// ! 논리부정 연산자. 값을 반전시킴. true는 false로 false는 true로 반전시킴
